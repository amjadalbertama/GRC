        ...
        <div class="col-12">
          <div class="row mb-3">
            <div class="col-12 col-md-8 col-lg-9 col-xl-10">
              ...
              @endif
              <a href="{{ route('export_controls') }}" class="btn btn-sm btn-secondary px-4 mr-2"><i class="fa fa-download mr-2"></i>Download</a>
            </div><!-- .col -->
              ...
              <div class="card bg-light rounded-xl rounded-lg border-bottom-0 bg-white">
                <div class="card-body px-3 py-1">
                  <form class="form-inline" action="javascript:void(0);">
                    <label for="sel_con_fil3" class="mt-2 mt-sm-0 mr-sm-2">Status:</label>
                    <select class="form-control form-control-sm mr-sm-2 bg-transparent border-0 px-0" id="sel_con_fil2" onchange="window.location.href = this.value">
                        <option value="{{ url('controls') }}" @if(!isset(Request()->status)) selected @endif>All</option>
                        @foreach($status_mapping as $status)
                        <option value="{{ url('controls') }}?status={{ $status->id }}" @if(isset(Request()->status) && Request()->status == $status->id) selected @endif>{{ $status->status }}</option>
                        @endforeach
                    </select>
                  </form>
                </div>
                ...
                <table class="table table-striped table-sm border bg-white">
                  ...
                  <tbody class="text-nowrap">
                    ...
                    <tr>
                      ...
                      <td>{{ $data['id'] }}</td>
                      <td class="w-500px pr-5 truncate-text"><a class="d-block text-truncate w-500px" href="{{ route('controlActivity', $data['id']) }}" title="{{ $data['title'] }}">{{ $data['title'] }}</a></td>
                      ...
                      <td class="text-nowrap">
                        <div class="dropdown">
                          ...
                          <div class="dropdown-menu dropdown-menu-right">
                            <a class="dropdown-item" href="{{ route('controlActivity', $data['id']) }}" title="View Control Activities"><i class="fa fa-search fa-fw mr-1"></i> View Control Activities</a>
                            @if($access['reviewer'] == true || $access['approval'] == true)
                            <a class="dropdown-item" href="javascript:void(0);" data-toggle="modal" onclick="approvalModal('{{ $data['id_program'] }}')" title="Check"><i class="fa fa-check fa-fw mr-1"></i> Check</a>
                            @endif
                            @if($access['delete'] == true)
                            <a class="dropdown-item" href="javascript:void(0);" data-toggle="modal" onclick="confirmationModal()" title="Delete"><i class="fa fa-trash fa-fw mr-1"></i> Delete</a>
                            @endif
